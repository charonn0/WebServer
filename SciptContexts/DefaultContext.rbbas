...
Implements ScriptContext
	#tag Method, Flags = &h1000
		Sub Constructor(PreCompile As Boolean = True)
		  // Calling the overridden superclass constructor.
		  // Constructor(Precompile As Boolean = True) -- From ScriptResponse
		  Super.Constructor(PreCompile)
	...
	#tag Method, Flags = &h0
		Function ContextComment() As String
		  Return "Minimal runtime and helper scripts"
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function IsMethodAvailable(MethodName As String) As Boolean
		  Return MethodName = "_RuntimeGetDictValue" Or MethodName = "_RuntimeGetNewDict" Or MethodName = "_RuntimeGetNewMB" Or MethodName = "_RuntimeSetDictValue"
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function PreParse(Source As String) As String
		  source = ReplaceAll(Source, "#Include Default.RTOBJECT", "")
		  Dim i As Integer = InStr(Source, "#Include Default.MemoryBlock")
		  Dim h As String = ReplaceLineEndings(_Class_RTOBJECT, EndOfLine.Windows) + EndOfLine.Windows
		  If i > 0 Then
		    source = ReplaceAll(Source, "#Include Default.MemoryBlock", "")
		    h = h + _Class_Memoryblock + EndOfLine
		  End If
		  
		  i = InStr(Source, "#Include Default.Struct")
		  If i > 0 Then
		    source = ReplaceAll(Source, "#Include Default.Struct", "")
		    h = h + _Class_Struct + EndOfLine
		  End If
	...
	#tag Method, Flags = &h0
		Function _RuntimeGetDictValue(Name As String, Key As String) As String
		  Return RTObjects.Value(Key)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function _RuntimeGetNewDict() As String
		  Dim dd As New Dictionary
		  RTObjects.Value("NEWDICT") = dd
		  Return "NEWDICT"
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function _RuntimeGetNewMB(Size As Integer, LittleEndian As Boolean) As String
		  Dim mb As New MemoryBlock(size)
		  mb.LittleEndian = LittleEndian
		  RTObjects.Value("NEWMB") = mb
		  Return "NEWMB"
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub _RuntimeSetDictValue(Name As String, Key As String, NewValue As String)
		  RTObjects.Value(Key) = NewValue
		End Sub
	#tag EndMethod


	#tag Property, Flags = &h0
		HeaderLineCount As Integer
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mRTObjects As Dictionary
	#tag EndProperty

	#tag ComputedProperty, Flags = &h1
		#tag Getter
			Get
			  If mRTObjects = Nil Then mRTObjects = New Dictionary
			  return mRTObjects
			End Get
		#tag EndGetter
		#tag Setter
			...
			End Set
		#tag EndSetter
		Protected RTObjects As Dictionary
	#tag EndComputedProperty


	#tag Constant, Name = _Class_MemoryBlock, Type = String, Dynamic = False, Default = \"Class MemoryBlock\rInherits _RuntimeObject\r  Sub Constructor(Size As Integer\x2C LittleEndian As Boolean \x3D TargetLittleEndian)\r    Super.Constructor(_RuntimeGetNewMB(Size\x2C LittleEndian))\r  End Sub\rEnd Class", Scope = Private
	#tag EndConstant

	#tag Constant, Name = _Class_RTOBJECT, Type = String, Dynamic = False, Default = \"Class _RuntimeObject\r  Protected Name As Variant\r  Sub Constructor(Name As Variant)\r    Me.Name \x3D Name\r  End Sub\r End Class", Scope = Private
	#tag EndConstant

	#tag Constant, Name = _Class_Struct, Type = String, Dynamic = False, Default = \"Class Struct\rInherits _RuntimeObject\r  Sub Constructor()\r    Dim n As Variant \x3D _RuntimeGetNewDict\r    Super.Constructor(n)\r  End Sub\r  Function Value(Key As Variant) As Variant\r\tReturn _RuntimeGetDictValue(Me.Name\x2C Key)\r  End Function\r  Sub Value(Key As Variant\x2C NewValue As Variant)\r\t_RuntimeSetDictValue(Me.Name\x2C Key\x2C NewValue)\r  End Sub\rEnd Class", Scope = Private
	#tag EndConstant
