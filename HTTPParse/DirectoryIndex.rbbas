...
Protected Class DirectoryIndex
	...
	#tag Method, Flags = &h21
		Private Function DoSorts(SubSort As Integer) As FolderItem()
		...
		Function ToString() As String
		  ...
		  For i As Integer = 0 To UBound(RequestPath.Arguments)
		    ...
		    Case "Sort"
		      Select Case v
		      Case "A"
		        Items = DoSorts(Sort_Alpha)
		      Case "D"
		        Items = DoSorts(Sort_Date)
		      Case "S"
		        Items = DoSorts(Sort_Size)
		      Case "T"
		        Items = DoSorts(Sort_Type)
		      Else
		        Items = DoSorts(Sort_Alpha)
		      End Select
		    End Select
		  Next
		  If Items.Ubound = -1 Then Items = DoSorts(Sort_Alpha)
		  Dim lines() As String
