#tag Class
Protected Class ScriptResponse
Inherits HTTP.Response
	#tag Method, Flags = &h21
		Private Sub CompilerError(Sender As RbScript, line As Integer, errorNumber As Integer, errorMsg As String)
		  #pragma Unused errorMsg
		  #pragma Unused Sender
		  Dim s() As String
		  RaiseEvent ScriptError(Line, ErrorNumber, False, s)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function InputHandler(Sender As RbScript, prompt As String) As String
		  #pragma Unused Sender
		  Return RaiseEvent ScriptInput(prompt)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub PrintHandler(Sender As RbScript, msg As String)
		  #pragma Unused Sender
		  RaiseEvent ScriptPrint(msg)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub RuntimeError(Sender As RbScript, line As Integer, error As RuntimeException)
		  #pragma Unused Sender
		  Dim s() As String = error.Stack
		  RaiseEvent ScriptError(Line, error.ErrorNumber, False, s)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		 Shared Function ScriptErrorMessage(code as integer) As string
	...
	#tag Hook, Flags = &h0
		Event GetSource() As String
	#tag EndHook

	#tag Hook, Flags = &h0
		Event ScriptError(Line As Integer, Code As Integer, RuntimeError As Boolean, Stack() As String)
	#tag EndHook

	#tag Hook, Flags = &h0
		Event ScriptInput(Prompt As String) As String
	#tag EndHook

	#tag Hook, Flags = &h0
		Event ScriptPrint(Data As String)
	#tag EndHook
	...
	#tag Property, Flags = &h1
		Protected ScriptHost As RbScript
	#tag EndProperty
